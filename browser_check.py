...
with sync_playwright() as p:
    ...
    expect(page.get_by_role('heading',name='Your energy network.')).to_be_visible(timeout=15000)
    ...
    page.route('**/api/locations/search?**', lambda route: route.fulfill(json={'results':[{'id':1,'name':'Leporiang','label':'Leporiang, Arunachal Pradesh','state':'Arunachal Pradesh','district':'Papum Pare','latitude':27.2297,'longitude':93.3412,'timezone':'Asia/Kolkata','state_verified':True}]}))
    page.get_by_label('Search location',exact=True).fill('Leporiang')
    page.get_by_role('option',name='Leporiang, Arunachal Pradesh').click()
    ...
    expect(page.locator('.notice[role=status]')).to_contain_text('Reading saved',timeout=15000)
